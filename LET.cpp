...
LET::LET(const index_t nSurface, const vector<Point> &location, double *const charge, const ulong N, const index_t rank) {
    this->rank = rank;
    this->N = N;
    this->maxLevel = 0;
    ...
    getStandardSurface(nSurface, 4.0 - sqrt(2) - 2.0 * d, standardUpCheckSurface);
    getStandardSurface(nSurface, sqrt(2) + d, standardUpEquivalentSurface);
    getStandardSurface(nSurface, sqrt(2) + d, standardDownCheckSurface);
    getStandardSurface(nSurface, 4.0 - sqrt(2) - 2.0 * d, standardDownEquivalentSurface);
    ...
    this->kernelFunction = [](Point& a, Point& b) {return  b.y * b.y;};
