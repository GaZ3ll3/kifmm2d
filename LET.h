...
class LET {
public:
    LET(const index_t nSurface, const vector<Point>& location, double* const charge, const ulong N, const index_t rank);
    ~LET();

    std::function<scalar_t(Point&, Point&)> kernelFunction;
